...
class KDEFPairedDataset(Dataset):
    """
    Dataset for KDEF paired facial expressions
    ...
    Args:
        data_path: Path to aligned KDEF dataset pickle file
    ...
    def __init__(self, data_path, 
        ...
        # Filter only KDEF entries
        self.df = self.df[self.df['dataset'] == 'KDEF'].reset_index(drop=True)
        ...
        # Create pairs of [neutral, non-neutral] images with same subject
        self.pairs = []
        for subject_id, emotions in self.subject_images.items():
            # Check if this subject has neutral images
            if 'neutral' in emotions and len(emotions['neutral']) > 0:
                neutral_indices = emotions['neutral']
                ...
                for neutral_idx in neutral_indices:
                    ...
                    if self.target_emotion:
                        ...
                        # Include all non-neutral emotions (original behavior)
        ...
        if self.target_emotion:
            emotion_desc = f"neutral-to-{'/'.join(self.target_emotion)}"
        else:
            emotion_desc = "neutral-to-emotion"
        ...
        unique_subjects = len(self.subject_images)
        
        print(f"Created {len(self.pairs)} {emotion_desc} pairs for {unique_subjects} subjects")
...
class KDEFNeutralHappyDataset(KDEFPairedDataset):
    """
...
def create_dataloaders(data_path,
                       ...
                       train_ratio=0.9):
    """
    Create DataLoaders for KDEF dataset with on-the-fly 90-10 split
    
    Args:
        data_path: Path to aligned_kdef_data.pkl file
        image_size: Size to resize images to
    ...
    # Filter only KDEF entries
    df = df[df['dataset'] == 'KDEF'].reset_index(drop=True)
